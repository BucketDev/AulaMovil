...
export class AssistanceCardPage {
  ...
  updateSelectedCheckBox = () => this.assistanceCheckbox.forEach((checkBox: IonCheckbox) => {
    const assistance = this.assistanceCards
      .find((assistanceCard: AssistanceCard) => checkBox.name === assistanceCard.assistanceUid);
    checkBox.checked = assistance ? assistance.notAssisted : false;
  })
